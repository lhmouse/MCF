...
wchar_t *wcschr(const wchar_t *s, wchar_t c){
	...
	for(;;){
		const __m128i xw1 = _mm_load_si128((const __m128i *)rp);
		const __m128i xw2 = _mm_load_si128((const __m128i *)rp);
		__m128i xt = _mm_packs_epi16(_mm_cmpeq_epi16(xw1, xc), _mm_cmpeq_epi16(xw2, xc));
		unsigned mask = (unsigned)_mm_movemask_epi8(xt);
		if(_MCFCRT_EXPECT_NOT(mask != 0)){
			return (wchar_t *)rp + __builtin_ctz(mask);
		}
		xt = _mm_packs_epi16(_mm_cmpeq_epi16(xw1, xz), _mm_cmpeq_epi16(xw2, xz));
