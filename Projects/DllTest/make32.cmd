...
g++ %CXXFlags% -std=c++17 -Wnoexcept -Woverloaded-virtual -fnothrow-opt -Wall -Wextra -pedantic -pedantic-errors -Wwrite-strings -Wsign-conversion -Wsuggest-attribute=noreturn -pipe -mfpmath=both -march=nocona -mno-stack-arg-probe -mno-accumulate-outgoing-args -mpush-args -masm=intel dll.cpp -shared -o ".dll-%Config%.dll" -I../.. -L../../.Built/%Config% -nostdlib -Wl,-e@__MCFCRT_DllStartup,--disable-runtime-pseudo-reloc %Lib% || exit /b 1

g++ dlltest.cpp ".dll-%Config%.dll" -std=c++17 -o ".%Config%.exe" -O3 -Wl,-s
