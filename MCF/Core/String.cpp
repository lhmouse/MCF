...
VVector<wchar_t> AnsiString::xUnify() const {
	...
	const std::size_t uUnifiedLen = ::MultiByteToWideChar(CP_ACP, 0, GetBegin(), GetLength(), nullptr, 0);
	if(uUnifiedLen > 0){
		vecUnified.Resize(uUnifiedLen);
		vecUnified.Resize(::MultiByteToWideChar(CP_ACP, 0, GetBegin(), GetLength(), vecUnified.GetData(), vecUnified.GetSize()));
	}
	return std::move(vecUnified);
}
template<>
void AnsiString::xDeunifyAppend(const VVector<wchar_t> &vecUnified){
	const std::size_t uDeunifiedLen = ::WideCharToMultiByte(CP_ACP, 0, vecUnified.GetBegin(), vecUnified.GetSize(), nullptr, 0, nullptr, nullptr);
	if(uDeunifiedLen > 0){
		const auto uOldLength = GetLength();
		Resize(uOldLength + uDeunifiedLen);
		Resize(uOldLength + ::WideCharToMultiByte(CP_ACP, 0, vecUnified.GetBegin(), vecUnified.GetSize(), GetData(), GetSize(), nullptr, nullptr));
	}
}
...
VVector<wchar_t> Utf8String::xUnify() const {
	...
	while(pchCur != pchEnd){
		...
		if((u32CodePoint & 0x80) == 0){
			// ASCII 字符。
			vecUnified.Push(u32CodePoint);
			continue;
		}
		// 这个值是该码点剩余的字节数，不包含刚刚读取的。
		const std::size_t uAddnlBytes = __builtin_clzl(~(u32CodePoint << (std::numeric_limits<unsigned int>::digits - 7)) | 1);
		...
		vecUnified.Push((u32CodePoint >> 10)   | 0xD800);
		vecUnified.Push((u32CodePoint & 0x3FF) | 0xDC00);
	}
...
void Utf8String::xDeunifyAppend(const VVector<wchar_t> &vecUnified){
	...
	while(pwcCur != pwcEnd){
		...
		if(u32CodePoint - 0xD800 <= 0x7FF){
			// 接受一个代理。
			u32CodePoint -= 0xD800;
			if(u32CodePoint > 0x3FF){
				// 这是个末代理。
				u32CodePoint = 0xFFFD;
				goto jEncode;
			}
			...
			const std::uint32_t u32Next = (std::uint16_t)*(pwcCur++) - 0xDC00;
			if(u32Next > 0x3FF){
				// 末代理无效。
				u32CodePoint = 0xFFFD;
				goto jEncode;
	...
	jEncode:
		// 这个值是该码点的总字节数减一。
		const std::size_t uAddnlBytes = (29 - __builtin_clzl(u32CodePoint | 1)) / 5;
		if(uAddnlBytes == 0){
			ASSERT(u32CodePoint <= 0x7F);

			Push(u32CodePoint);
			continue;
		}
		u32CodePoint |= (std::uint32_t)0xFFFFFF80 << (uAddnlBytes * 5);
		Push(u32CodePoint >> (uAddnlBytes * 6));
		for(std::size_t i = uAddnlBytes; i != 0; --i){
			Push(((u32CodePoint >> (i * 6 - 6)) & 0x3F) | 0x80);
		}
	}
}
