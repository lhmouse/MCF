...
public:
	...
	Element *Erase(const Element *pBegin, const Element *pEnd) noexcept(noexcept(DeclVal<FlatMultiSet &>().x_vecStorage.Erase(pBegin, pEnd))) {
		return x_vecStorage.Erase(pBegin, pEnd);
	}
	Element *Erase(const Element *pPos) noexcept(noexcept(DeclVal<FlatMultiSet &>().x_vecStorage.Erase(pPos))) {
