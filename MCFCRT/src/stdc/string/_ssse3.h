...
__attribute__((__selectany__, __aligned__(32))) extern const uint8_t __MCFCRT_xmmalign_masks[16][32];
...
__attribute__((__always_inline__)) static inline void __MCFCRT_xmmalign_2(__m128i *_MCFCRT_RESTRICT __words, const __m128i *_MCFCRT_RESTRICT __view, unsigned __align) _MCFCRT_NOEXCEPT {
	const __m128i *const __align_masks = (const __m128i *)(const void *)__MCFCRT_xmmalign_masks[__align % 16];
	for(unsigned __i = 0; __i < 2; ++__i){
		__words[__i] = _mm_or_si128(_mm_shuffle_epi8(__view[__align / 16 + __i + 0], _mm_load_si128(__align_masks + 0)),
		                            _mm_shuffle_epi8(__view[__align / 16 + __i + 1], _mm_load_si128(__align_masks + 1)));
	}
}

__attribute__((__always_inline__)) static inline void __MCFCRT_xmmalign_4(__m128i *_MCFCRT_RESTRICT __words, const __m128i *_MCFCRT_RESTRICT __view, unsigned __align) _MCFCRT_NOEXCEPT {
	const __m128i *const __align_masks = (const __m128i *)(const void *)__MCFCRT_xmmalign_masks[__align % 8 * 2];
	for(unsigned __i = 0; __i < 4; ++__i){
		__words[__i] = _mm_or_si128(_mm_shuffle_epi8(__view[__align / 8 + __i + 0], _mm_load_si128(__align_masks + 0)),
		                            _mm_shuffle_epi8(__view[__align / 8 + __i + 1], _mm_load_si128(__align_masks + 1)));
	}
}
