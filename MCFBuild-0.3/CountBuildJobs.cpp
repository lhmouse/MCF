				...
				if(llGCHSrcTimestamp == LLONG_MIN){
					...
					if(bRebuildAll){
						...
						if(llGCHTimestamp == LLONG_MIN){
							Output(L"      预编译头文件“" + wcsGCHStub + L".gch”不存在。");
							bNeedRebuilding = true;
					...
					if(bNeedRebuilding){
						if(bVerbose){
							Output(L"      源文件：" + wcsGCHSrc);
							Output(L"      桩文件：" + wcsGCHStub);
						}
						Output(L"      将构建预编译头文件“" + wcsGCHStub + L".gch”。");
						bRebuildGCH = true;

						GCHItem.wcsSourceFile	= std::move(wcsGCHSrc);
						GCHItem.wcsCommandLine	= PreCompiledHeaderItem.second.wcsCommandLine;
					} else {
						Output(L"      预编译头文件“" + wcsGCHStub + L".gch”已为最新。");
