...
void DependencyDatabase::SaveToFile(const std::wstring &wcsPath) const {
...
void DependencyDatabase::LoadFromFile(const std::wstring &wcsPath){
	...
	std::map<std::wstring, Dependencies> mapDependencies;
	...
	for(std::uint32_t i = 0; i < u32MapSize; ++i){
		...
		std::wstring wcsKey(u32KeyLength, 0);
		...
		for(std::uint32_t j = 0; j < u32SetSize; ++j){
			...
			std::wstring wcsFile(u32FileLength, 0);
