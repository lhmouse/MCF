...
public:
	...
	std::size_t Read(void *pBuffer, std::uint32_t u32BytesToRead, std::uint64_t u64Offset,
		const FunctionObserver<void ()> &fnAsyncProc = nullptr, const FunctionObserver<void ()> &fnCompleteCallback = nullptr) const;
	std::size_t Write(std::uint64_t u64Offset, const void *pBuffer, std::uint32_t u32BytesToWrite,
		const FunctionObserver<void ()> &fnAsyncProc = nullptr, const FunctionObserver<void ()> &fnCompleteCallback = nullptr);
	void Flush() const;
