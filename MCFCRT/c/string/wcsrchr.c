...
__MCF_CRT_EXTERN wchar_t *wcsrchr(const wchar_t *s, wchar_t ch){
	wchar_t *ret = NULL;
...
#define UNROLLED(index)	\
		{	\
			...
			register uintptr_t bak = wrd;	\
			wrd ^= full;	\
			wrd = (wrd - MASK) & ~wrd;	\
			if(((wrd | wrd2) & (MASK << 15)) != 0){	\
				for(size_t i = 0; i < sizeof(uintptr_t) / sizeof(wchar_t); ++i){	\
					if((bak & 0xFFFF) == (full & 0xFFFF)){	\
						ret = (wchar_t *)(rp + (index) * sizeof(uintptr_t) / sizeof(wchar_t) + i);	\
					}	\
					if((wrd2 & 0x8000) != 0){	\
						return ret;	\
					}	\
