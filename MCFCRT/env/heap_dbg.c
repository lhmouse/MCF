...
static MCFCRT_AvlRoot  g_pavlBlocks;
...
void __MCFCRT_HeapDbgUninit(){
	...
	if(g_pavlBlocks){
		...
		const __MCFCRT_HeapDbgBlockInfo *pBlockInfo = (const __MCFCRT_HeapDbgBlockInfo *)MCFCRT_AvlFront(&g_pavlBlocks);
		do {
...
unsigned char *__MCFCRT_HeapDbgRegisterBlockInfo(__MCFCRT_HeapDbgBlockInfo *pBlockInfo, unsigned char *pRaw, MCFCRT_STD size_t uContentSize, const void *pRetAddr){
	...
	MCFCRT_AvlAttach(&g_pavlBlocks, (MCFCRT_AvlNodeHeader *)pBlockInfo, &BlockInfoComparatorNodes);
...
__MCFCRT_HeapDbgBlockInfo *__MCFCRT_HeapDbgValidateBlock(unsigned char **ppRaw, unsigned char *pContents, const void *pRetAddr){
	...
	*ppRaw = pRaw;
	...
	__MCFCRT_HeapDbgBlockInfo *const pBlockInfo = (__MCFCRT_HeapDbgBlockInfo *)MCFCRT_AvlFind(&g_pavlBlocks, (intptr_t)pContents, &BlockInfoComparatorNodeKey);
	if(!pBlockInfo){
