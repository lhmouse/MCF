...
void StandardErrorStream::Put(unsigned char byData){
	...
	if(!bWritten){
		...
		MCF_THROW(Exception, dwErrorCode, Rcntws::View(L"StandardErrorStream: _MCFCRT_WriteStandardErrorByte() 失败。"));
	}
}
void StandardErrorStream::Put(const void *pData, std::size_t uSize){
	...
	if(uWritten == 0){
		const auto dwErrorCode = ::GetLastError();
		MCF_THROW(Exception, dwErrorCode, Rcntws::View(L"StandardErrorStream: _MCFCRT_WriteStandardErrorBinary() 失败。"));
	}
	if(uWritten < uSize){
...
void StandardErrorStream::PutChar32(char32_t c32Data){
	...
	if(!bWritten){
		const auto dwErrorCode = ::GetLastError();
		MCF_THROW(Exception, dwErrorCode, Rcntws::View(L"StandardErrorStream: _MCFCRT_WriteStandardErrorChar32() 失败。"));
	}
}
void StandardErrorStream::PutString(const wchar_t *pwcData, std::size_t uSize, bool bAppendNewLine){
	...
	if(uWritten == 0){
		const auto dwErrorCode = ::GetLastError();
		MCF_THROW(Exception, dwErrorCode, Rcntws::View(L"StandardErrorStream: _MCFCRT_WriteStandardErrorString() 失败。"));
	}
	if(uWritten < uSize){
		MCF_THROW(Exception, ERROR_BROKEN_PIPE, Rcntws::View(L"StandardErrorStream: 未能成功写入所有数据。"));
	}
