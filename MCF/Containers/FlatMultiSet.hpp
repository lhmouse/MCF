...
public:
	...
	Element *Erase(const Element *pBegin, const Element *pEnd) noexcept(noexcept(DeclVal<FlatMultiSet &>().x_vStorage.Erase(pBegin, pEnd))) {
		return x_vStorage.Erase(pBegin, pEnd);
	}
	Element *Erase(const Element *pPos) noexcept(noexcept(DeclVal<FlatMultiSet &>().x_vStorage.Erase(pPos))) {
