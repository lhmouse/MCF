...
void Serialize(StreamBuffer &sbufSink, const std::list<Element, Allocator> &vSource){
	const auto uSize = vSource.size();
	SerializeSize(sbufSink, uSize);
	Serialize<Element>(sbufSink, vSource.begin(), uSize);
}
template<typename Element, class Allocator>
void Deserialize(std::list<Element, Allocator> &vSink, StreamBuffer &sbufSource){
	vSink.clear();
